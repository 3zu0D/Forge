...
function exportActiveProjectToExcel() {
    ...
    const stakeholderRows = [
        ["N°", "Nom", "Prénom", "Fonction", "Rôle", "Responsabilité"]
    ];
    ...
    const raciRows = buildRaciRowsForExcel(exportStakeholders, exportPhases, exportWbsRows, exportRaci);
    ...
    const workbookXml = buildExcelWorkbookXml([
        ...
        },
        {
            ...
            widths: [45, 100, 100, 160, 140, 360]
        },
        {
            name: "WBS",
            rows: wbsRowsForExcel,
            widths: [45, 140, 280, 150, 75, 75, 75, 95, 95, 75, 320]
        },
        {
            name: "RACI",
            rows: raciRows,
            widths: buildRaciWidths(exportStakeholders)
        }
    ]);
    ...
    const date = new Date().toISOString().slice(0, 10);
    const fileName = `forge-${safeProjectName}-${date}.xls`;
    const downloadUrl = URL.createObjectURL(blob);
    const link = document.createElement("a");
...
function initWbsPage() {
    ...
    addPhaseButton.addEventListener("click", () => {
        ...
        const lastPhase = phasesTableBody.querySelector("tr:last-child .editable");
        if (lastPhase) {
            lastPhase.focus();
        }
    });

    deleteSelectedPhasesButton.addEventListener("click", () => {
    ...
    resetPhasesButton.addEventListener("click", () => {
    ...
    selectAllPhasesCheckbox.addEventListener("change", (event) => {
        selectedPhases.clear();
    ...
    addWbsRowButton.addEventListener("click", () => {
        wbsRows.push({
            id: createId(),
            phaseId: phases[0]?.id || "",
            task: "",
    ...
    deleteSelectedWbsButton.addEventListener("click", () => {
    ...
    resetWbsButton.addEventListener("click", () => {
        const confirmation = confirm("Tu veux vraiment réinitialiser le WBS ?");
        if (!confirmation) return;

        wbsRows = structuredClone(defaultWbsRows);
        selectedWbsRows.clear();
        saveWbsRows();
        renderWbsTable();
    });

    selectAllWbsCheckbox.addEventListener("change", (event) => {
        selectedWbsRows.clear();
...
function renderWbsTable() {
    ...
    if (wbsRows.length === 0) {
        const row = document.createElement("tr");
        row.innerHTML = `<td colspan="12" class="empty-state">Aucune ligne WBS pour le moment.</td>`;
        wbsTableBody.appendChild(row);
        updateWbsSelectionControls();
        return;
    }

    wbsRows.forEach((wbsRow, index) => {
        const phase = phases.find((item) => item.id === wbsRow.phaseId);
        const color = phase ? normalizeColor(phase.color, index) : "#94a3b8";
        const isSelected = selectedWbsRows.has(index);
        const row = document.createElement("tr");

        row.style.backgroundColor = hexToRgba(color, 0.26);
        row.style.boxShadow = `inset 3px 0 0 ${color}`;
        ...
        row.innerHTML = `
            <td class="select-col">
                <input
                    ...
                    type="checkbox"
                    data-index="${index}"
                    aria-label="Sélectionner la ligne WBS ${index + 1}"
                    ${isSelected ? "checked" : ""}
                />
            </td>
            <td>${index + 1}</td>
            <td>
                <select class="phase-select" data-index="${index}" data-field="phaseId">
                    <option value="">Sans phase</option>
                    ${createPhaseOptions(wbsRow.phaseId)}
                </select>
            </td>
            ${createWbsEditableCell(wbsRow.task, index, "task", "wbs-task-cell")}
            ${createWbsEditableCell(wbsRow.responsable, index, "responsable")}
            ${createWbsEditableCell(wbsRow.jourDebut, index, "jourDebut", "wbs-short-cell")}
            ${createWbsEditableCell(wbsRow.duree, index, "duree", "wbs-short-cell")}
            ${createWbsEditableCell(wbsRow.jourFin, index, "jourFin", "wbs-short-cell")}
            <td class="wbs-date-cell">
                <input class="wbs-date-input" type="date" value="${escapeHtml(wbsRow.dateDebut)}" data-index="${index}" data-field="dateDebut" />
            </td>
            <td class="wbs-date-cell">
                <input class="wbs-date-input" type="date" value="${escapeHtml(wbsRow.dateFin)}" data-index="${index}" data-field="dateFin" />
            </td>
            <td class="wbs-short-cell">
                <input class="wbs-progress-input" type="number" min="0" max="100" value="${escapeHtml(wbsRow.avancement)}" data-index="${index}" data-field="avancement" />
            </td>
            ${createWbsEditableCell(wbsRow.commentaire, index, "commentaire", "wbs-comment-cell")}
        `;
...
function bindWbsTableEvents() {
    ...
    wbsTableBody.querySelectorAll(".phase-select").forEach((select) => {
        select.addEventListener("change", (event) => {
            const index = Number(event.target.dataset.index);
            wbsRows[index].phaseId = event.target.value;
            saveWbsRows();
            renderWbsTable();
        });
    });

    wbsTableBody.querySelectorAll(".wbs-date-input, .wbs-progress-input").forEach((input) => {
        input.addEventListener("input", (event) => {
            ...
            wbsRows[index][field] = event.target.value;
            saveWbsRows();
        });
    });
...
function loadWbsRows() {
    ...
    if (!savedData) {
        return structuredClone(defaultWbsRows);
    }
    ...
    try {
        ...
        const normalizedWbsRows = parsedData.map((row) => ({
            id: row.id || createId(),
            phaseId: row.phaseId || "",
            task: row.task || "",
        ...
        localStorage.setItem(getProjectKey("wbs"), JSON.stringify(normalizedWbsRows));
        return normalizedWbsRows;
    } catch (error) {
...
function renderRaciTable() {
    ...
    groupedRows.forEach((group) => {
        ...
        phaseRow.className = "raci-phase-row";
        phaseRow.style.backgroundColor = hexToRgba(phaseColor, 0.32);
        ...
        group.rows.forEach(({ row, originalIndex }) => {
            ...
            taskRow.style.backgroundColor = hexToRgba(phaseColor, 0.18);
...
function groupWbsRowsByPhase() {
    ...
    wbsRows.forEach((row, originalIndex) => {
        const phaseId = row.phaseId || "__no_phase__";

        if (!groupMap.has(phaseId)) {
            const phase = phases.find((item) => item.id === row.phaseId) || null;
            const group = {
                phaseId,
                phase,
                rows: []
...
function closeColorMenuOnOutsideClick(event) {
    const clickedNumber = event.target.closest(".row-number-btn, .phase-number-btn");
...
function exportActiveProjectToExcel() {
    ...
    const workbookBlob = buildForgeXlsxWorkbook([
        ...
        },
        {
            name: "WBS",
            rows: wbsRowsForExcel,
            widths: [8, 22, 44, 24, 12, 12, 12, 15, 15, 12, 58],
            freezeHeader: true,
            autoFilter: true
        },
        {
            name: "RACI",
            rows: raciRows,
            widths: buildRaciXlsxWidths(exportStakeholders),
            freezeHeader: true,
            autoFilter: true,
            phaseRows: getRaciPhaseRowIndexesForXlsx(raciRows),
            raciStartColumn: 4
        }
    ]);
...
function renderGantt(ganttHead, ganttBody, ganttSummary) {
    ...
    let timelineStart = projectStart || getMinDate(validTasks.map((task) => task.start));
    let timelineEnd = projectEnd || getMaxDate(validTasks.map((task) => task.end));

    if (!timelineStart || !timelineEnd || timelineEnd < timelineStart) {
        ganttHead.innerHTML = "";
        ganttBody.innerHTML = `
            <tr>
                <td class="gantt-empty-state">
    ...
    const days = buildDateRange(timelineStart, timelineEnd);
    const groupedTasks = groupGanttTasksByPhase(plannedTasks);
    const totalColumns = 6 + days.length;

    ganttHead.innerHTML = buildGanttHeader(days);
    ganttBody.innerHTML = "";

    if (plannedTasks.length === 0) {
        ganttBody.innerHTML = `
            <tr>
                <td colspan="${totalColumns}" class="gantt-empty-state">
                    Ajoute des lignes dans le WBS pour générer les tâches du GANTT.
                </td>
            </tr>
        `;
        return;
    ...
    groupedTasks.forEach((group) => {
        ...
        const phaseRow = document.createElement("tr");
        phaseRow.className = "gantt-phase-row";
        phaseRow.style.setProperty("--gantt-color", phaseColor);
        phaseRow.style.setProperty("--gantt-color-soft", hexToRgba(phaseColor, 0.26));
        phaseRow.innerHTML = `
            <td colspan="${totalColumns}">
                <span class="gantt-phase-label">
                    <span class="gantt-phase-dot"></span>
                    ${escapeHtml(phaseName)}
                </span>
            </td>
        `;
        ...
        group.tasks.forEach((task) => {
            const taskRow = document.createElement("tr");
            taskRow.style.setProperty("--gantt-color", phaseColor);
            taskRow.style.setProperty("--gantt-color-soft", hexToRgba(phaseColor, 0.22));
            taskRow.style.setProperty("--gantt-color-strong", hexToRgba(phaseColor, 0.70));
            taskRow.innerHTML = buildGanttTaskRow(task, days, phaseColor);
            ganttBody.appendChild(taskRow);
        });
    });

    if (ganttSummary) {
        ganttSummary.textContent = `${formatForgeDate(timelineStart)} → ${formatForgeDate(timelineEnd)} · ${days.length} jour${days.length > 1 ? "s" : ""} · ${validTasks.length} tâche${validTasks.length > 1 ? "s" : ""} planifiée${validTasks.length > 1 ? "s" : ""}`;
    }
}
...
function buildGanttHeader(days) {
    ...
    const monthHeader = `
        <tr>
            ...
            <th class="gantt-sticky gantt-left-owner" rowspan="2">Responsable</th>
...
function buildGanttTaskRow(task, days) {
    ...
    return `
        ...
        <td class="gantt-sticky gantt-left-owner gantt-small-info">${escapeHtml(task.row.responsable || "—")}</td>
...
function buildGanttTask(row, originalIndex, projectStart) {
    ...
    if (!start && projectStart && jourDebut) {
        start = addForgeDays(projectStart, jourDebut - 1);
    }

    if (!end && projectStart && jourFin) {
        end = addForgeDays(projectStart, jourFin - 1);
    }

    if (!end && start && duree) {
        end = addForgeDays(start, Math.max(duree - 1, 0));
    }
...
function initWbsPage() {
    ...
    resetWbsButton.addEventListener("click", () => {
        ...
        wbsRows = structuredClone(defaultWbsRows).map((row) => ({
            ...row,
            id: row.id || createId(),
...
function renderWbsTable() {
    ...
    wbsRows.forEach((wbsRow, index) => {
        ...
        row.innerHTML = `
            ...
            ${createWbsEditableCell(wbsRow.task, index, "task", "wbs-task-cell")}
            <td>
                <select class="wbs-responsable-select" data-index="${index}" data-field="responsableId">
                    ${createWbsResponsableOptions(wbsRow)}
                </select>
            </td>
            ${createWbsEditableCell(wbsRow.jourDebut, index, "jourDebut", "wbs-short-cell wbs-schedule-cell")}
            ${createWbsEditableCell(wbsRow.duree, index, "duree", "wbs-short-cell wbs-schedule-cell")}
            ${createWbsEditableCell(wbsRow.jourFin, index, "jourFin", "wbs-short-cell wbs-schedule-cell")}
            <td class="wbs-date-cell">
                <input class="wbs-date-input" type="date" value="${escapeHtml(wbsRow.dateDebut)}" data-index="${index}" data-field="dateDebut" readonly />
            </td>
            <td class="wbs-date-cell">
                <input class="wbs-date-input" type="date" value="${escapeHtml(wbsRow.dateFin)}" data-index="${index}" data-field="dateFin" readonly />
            </td>
            <td class="wbs-short-cell">
                <input class="wbs-progress-input" type="number" min="0" max="100" value="${escapeHtml(wbsRow.avancement)}" data-index="${index}" data-field="avancement" />
            </td>
...
function bindWbsTableEvents() {
    wbsTableBody.querySelectorAll(".editable").forEach((cell) => {
        cell.addEventListener("input", (event) => {
            ...
            if (!["jourDebut", "duree", "jourFin"].includes(field)) {
                saveWbsRows();
            }
        });
...
function loadWbsRows() {
    ...
    if (!savedData) {
        return applyWbsAutoSchedule(structuredClone(defaultWbsRows).map((row) => ({
            ...row,
            id: row.id || createId(),
            responsableId: row.responsableId || ""
        })));
    ...
    try {
        ...
        const scheduledRows = applyWbsAutoSchedule(normalizedWbsRows);
        localStorage.setItem(getProjectKey("wbs"), JSON.stringify(scheduledRows));
...
function recalculateWbsSchedule(shouldSave = false) {
    wbsRows = applyWbsAutoSchedule(wbsRows);
...
function applyWbsAutoSchedule(rows) {
    ...
    return rows.map((row) => {
        ...
        let startDay = parsePositiveInteger(output.jourDebut);
        const duration = parsePositiveInteger(output.duree);
        let endDay = parsePositiveInteger(output.jourFin);

        if (!startDay) {
            startDay = previousEndDay > 0 ? previousEndDay + 1 : 1;
            output.jourDebut = String(startDay);
        }

        if (duration) {
            endDay = startDay + duration - 1;
            output.jourFin = String(endDay);
        } else if (!endDay) {
            output.jourFin = "";
        }

        if (projectStart && startDay) {
            output.dateDebut = formatForgeInputDate(addForgeWorkingDays(projectStart, startDay - 1, settings.weekendsWorked));
        } else {
            output.dateDebut = "";
        }

        if (projectStart && endDay) {
            output.dateFin = formatForgeInputDate(addForgeWorkingDays(projectStart, endDay - 1, settings.weekendsWorked));
        } else {
            output.dateFin = "";
        }

        previousEndDay = endDay || startDay || previousEndDay;
...
function renderWbsTable() {
    ...
    wbsRows.forEach((wbsRow, index) => {
        ...
        row.innerHTML = `
            ...
            <td class="wbs-short-cell wbs-computed-cell" title="Calculé automatiquement">${escapeHtml(wbsRow.jourDebut || "")}</td>
            ${createWbsEditableCell(wbsRow.duree, index, "duree", "wbs-short-cell wbs-duration-cell")}
            <td class="wbs-short-cell wbs-computed-cell" title="Calculé automatiquement">${escapeHtml(wbsRow.jourFin || "")}</td>
            <td class="wbs-date-cell">
                <input class="wbs-date-input" type="date" value="${escapeHtml(wbsRow.dateDebut)}" data-index="${index}" data-field="dateDebut" readonly />
            </td>
            <td class="wbs-date-cell">
...
function applyWbsAutoSchedule(rows) {
    ...
    return rows.map((row) => {
        ...
        const duration = parsePositiveInteger(output.duree) || 1;
        const startDay = nextStartDay;
        const endDay = startDay + duration - 1;

        output.jourDebut = String(startDay);
        output.duree = String(duration);
        output.jourFin = String(endDay);
...
function initWbsPage() {
    ...
    addWbsRowButton.addEventListener("click", addWbsRowSequential);
...
function renderGantt(ganttHead, ganttBody, ganttSummary) {
    ...
    if (ganttSummary) {
        ...
        ganttSummary.textContent = `${phaseLabel} · ${formatForgeDate(timelineStart)} → ${formatForgeDate(timelineEnd)} · ${days.length} jour${days.length > 1 ? "s" : ""} · ${visibleValidTasks.length} tâche${visibleValidTasks.length > 1 ? "s" : ""} planifiée${visibleValidTasks.length > 1 ? "s" : ""}`;
    }
}
...
function loadForgeTheme() {
    return localStorage.getItem(FORGE_THEME_STORAGE_KEY) || "cyber";
}
...
function applyForgeTheme(theme) {
    document.body.dataset.theme = theme === "light" ? "light" : "cyber";
}
...
function initKpisPage() {
    ...
    selectAllTypes?.addEventListener("change", (event) => {
        ...
        if (event.target.checked) {
            kpiTypes.forEach((_, index) => selectedKpiTypes.add(index));
        }
    ...
    selectAllKpis?.addEventListener("change", (event) => {
        ...
        if (event.target.checked) {
            kpiRows.forEach((_, index) => selectedKpiRows.add(index));
        }
...
function renderKpiTable() {
    ...
    kpiRows.forEach((kpi, index) => {
        ...
        row.innerHTML = `
            <td class="select-col">
                <input
                    class="row-checkbox kpi-checkbox"
                    type="checkbox"
                    data-index="${index}"
                    aria-label="Sélectionner le KPI ${index + 1}"
                    ${isSelected ? "checked" : ""}
                />
            </td>
            <td>${index + 1}</td>
...
function bindKpiTableEvents() {
    ...
    body.querySelectorAll(".kpi-type-select, .kpi-status-select, .kpi-frequency-select").forEach((select) => {
        select.addEventListener("change", (event) => {
            const index = Number(event.target.dataset.index);
            const field = event.target.dataset.field;
...
function closeColorMenuOnOutsideClick(event) {
    const clickedNumber = event.target.closest(".row-number-btn, .phase-number-btn, .kpi-type-number-btn");
    const clickedMenu = event.target.closest("#color-menu");
...
function importForgeData(event) {
    ...
    reader.onload = () => {
        try {
            ...
            if (payload.theme) {
                saveForgeTheme(normalizeForgeTheme(payload.theme));
            }
...
function exportActiveProjectToExcel() {
    ...
    exportKpiTypes.forEach((type, index) => {
        kpiTypeRows.push([index + 1, type.name || ""]);
    });

    const kpiRowsForExcel = [
        ["N°", "Type", "KPI", "Objectif", "Unité", "Cible", "Actuel", "Écart", "Statut", "Fréquence", "Responsable", "Commentaire"]
    ];
    ...
    const workbookBlob = buildForgeXlsxWorkbook([
        ...
        {
            name: "KPIs",
            rows: kpiRowsForExcel,
            widths: [8, 22, 34, 54, 12, 14, 14, 14, 18, 18, 24, 54],
            freezeHeader: true,
            autoFilter: true
        },
        {
...
function renderRisksTable() {
    ...
    if (riskRows.length === 0) {
        body.innerHTML = `<tr><td colspan="12" class="empty-state">Aucun risque pour le moment. Clique sur “Ajouter un risque” pour commencer.</td></tr>`;
        if (deleteButton) deleteButton.disabled = true;
        if (selectAll) selectAll.checked = false;
        return;
    }
    ...
    riskRows.forEach((risk, index) => {
        ...
        row.innerHTML = `
            ...
            ${createRiskEditableCell(risk.cause, index, "cause", "risk-cause-cell")}
            <td>
            ...
            <td>
                <select class="risk-status-select" data-index="${index}" data-field="status">
                    ${createRiskStatusOptions(risk.status)}
                </select>
            </td>
            ${createRiskEditableCell(risk.comment, index, "comment", "risk-comment-cell")}
        `;
...
function bindRisksTableEvents() {
    ...
    body.querySelectorAll(".risk-type-select, .risk-probability-select, .risk-severity-select, .risk-response-select, .risk-status-select").forEach((select) => {
        select.addEventListener("change", (event) => {
            const index = Number(event.target.dataset.index);
            const field = event.target.dataset.field;
...
function closeColorMenuOnOutsideClick(event) {
    const clickedNumber = event.target.closest(".row-number-btn, .phase-number-btn, .kpi-type-number-btn, .risk-type-number-btn");
    const clickedMenu = event.target.closest("#color-menu");
...
function exportActiveProjectToExcel() {
    ...
    exportRiskTypes.forEach((type, index) => {
        riskTypeRows.push([index + 1, type.name || ""]);
    });

    const riskRowsForExcel = [
        ["N°", "Type", "Risque", "Cause", "Probabilité", "Gravité", "Criticité", "Réponse", "Responsable", "Statut", "Commentaire"]
    ];
    ...
    const workbookBlob = buildForgeXlsxWorkbook([
        ...
        {
            name: "Risques",
            rows: riskRowsForExcel,
            widths: [8, 22, 42, 42, 14, 14, 14, 18, 24, 18, 54],
            freezeHeader: true,
            autoFilter: true
        },
        {
...
function renderRiskMatrix() {
    ...
    for (let severity = 5; severity >= 1; severity -= 1) {
        for (let probability = 1; probability <= 5; probability += 1) {
            ...
            cells.push(`
                <div class="risk-matrix-cell ${level}" title="Probabilité ${probability} × Gravité ${severity}">
                    ${score}
                    <small>P${probability} · G${severity}</small>
                </div>
            `);
        }
    }
...
function exportActiveProjectToExcel() {
    ...
    const workbookBlob = buildForgeXlsxWorkbook([
        ...
        },
        {
            name: "Risques",
            rows: riskRowsForExcel,
            widths: [8, 22, 42, 42, 14, 14, 14, 18, 24, 64],
            freezeHeader: true,
            autoFilter: true
        },
        {
...
function renderRiskMatrix() {
        ...
        for (let probability = 1; probability <= 5; probability += 1) {
            ...
            cells.push(`
                <div class="risk-matrix-cell ${level}" title="Probabilité ${probability} × Gravité ${severity} = ${score} · ${label}">
                    <strong>${score}</strong>
                    <span>P${probability} × G${severity}</span>
    ...
    if (panel && !panel.querySelector(".risk-matrix-help")) {
        const help = document.createElement("p");
        help.className = "risk-matrix-help";
        help.textContent = "Lecture : P = probabilité, G = gravité. Le score correspond à P × G.";
...
function initSmartPage() {
    ...
    addButton?.addEventListener("click", () => {
        ...
        const lastObjective = document.querySelector("#smart-table-body tr:last-child .smart-objective-cell");
        if (lastObjective) lastObjective.focus();
    });

    deleteButton?.addEventListener("click", () => {
        if (selectedSmartRows.size === 0) return;

        const confirmation = confirm("Tu veux vraiment supprimer les objectifs SMART cochés ? Les KPIs liés passeront sans objectif.");
        if (!confirmation) return;

        const deletedIds = smartRows
            .filter((_, index) => selectedSmartRows.has(index))
            .map((row) => row.id);

        smartRows = smartRows.filter((_, index) => !selectedSmartRows.has(index));

        const kpis = loadKpis().map((kpi) => (
            deletedIds.includes(kpi.smartId)
                ? { ...kpi, smartId: "" }
                : kpi
        ));
    ...
    resetButton?.addEventListener("click", () => {
        const confirmation = confirm("Tu veux vraiment réinitialiser les objectifs SMART ?");
        if (!confirmation) return;
...
function loadSmartObjectives() {
    ...
    try {
        ...
        console.error("Impossible de charger les objectifs SMART :", error);
        return [];
    }
}
...
function renderSmartTable() {
    ...
    if (smartRows.length === 0) {
        body.innerHTML = `<tr><td colspan="12" class="empty-state">Aucun objectif SMART pour le moment. Clique sur “Ajouter un objectif” pour commencer.</td></tr>`;
        if (deleteButton) deleteButton.disabled = true;
        if (selectAll) selectAll.checked = false;
        return;
    }
    ...
    smartRows.forEach((smart, index) => {
        ...
        row.innerHTML = `
            <td class="select-col">
                <input class="row-checkbox smart-checkbox" type="checkbox" data-index="${index}" aria-label="Sélectionner l'objectif SMART ${index + 1}" ${isSelected ? "checked" : ""} />
            </td>
            <td>${index + 1}</td>
            ${createSmartEditableCell(smart.objective, index, "objective", "smart-objective-cell")}
...
function getSmartObjectiveLabel(objective) {
    const title = objective.objective || "Objectif sans nom";
    const dueDate = objective.dueDate ? ` · ${objective.dueDate}` : "";
...
function exportActiveProjectToExcel() {
    ...
    const smartRowsForExcel = [
        ["N°", "Objectif", "Spécifique", "Mesurable", "Atteignable", "Réaliste", "Temporel", "Échéance", "Responsable", "Statut", "Note"]
    ];
    ...
    const kpiRowsForExcel = [
        ["N°", "Objectif lié", "Type", "KPI", "Objectif KPI", "Unité", "Cible", "Actuel", "Écart", "Statut", "Fréquence", "Responsable"]
    ];
    exportKpis.forEach((kpi, index) => {
        const type = exportKpiTypes.find((item) => item.id === kpi.typeId);
...
function seedProjectData(projectId) {
    if (!localStorage.getItem(`forge_project_${projectId}_context_v1`)) {
        localStorage.setItem(`forge_project_${projectId}_context_v1`, JSON.stringify(structuredClone(CONTEXT_DEFAULT)));
    }
    if (!localStorage.getItem(`forge_project_${projectId}_stakeholders_v1`)) {
        localStorage.setItem(`forge_project_${projectId}_stakeholders_v1`, JSON.stringify([]));
    }
...
function loadProjectContext() {
    ...
    if (!savedData) {
        const defaults = structuredClone(CONTEXT_DEFAULT);
    ...
    try {
        ...
        return {
            ...structuredClone(CONTEXT_DEFAULT),
            ...(parsed && typeof parsed === "object" ? parsed : {})
        };
    } catch (error) {
        console.error("Impossible de charger le contexte du projet :", error);
        return structuredClone(CONTEXT_DEFAULT);
    }
}
...
function buildContextPlainText() {
    ...
    Object.entries(contextFieldLabels).forEach(([key, label]) => {
...
function exportForgeData() {
    ...
    exportPayload.projects.forEach((project) => {
        exportPayload.projectData[project.id] = {
            context: readJsonFromStorage(`forge_project_${project.id}_context_v1`, structuredClone(CONTEXT_DEFAULT)),
            stakeholders: readJsonFromStorage(`forge_project_${project.id}_stakeholders_v1`, []),
            competenceCategories: readJsonFromStorage(`forge_project_${project.id}_competence_categories_v1`, structuredClone(defaultCompetenceCategories)),
            competenceMatrices: readJsonFromStorage(`forge_project_${project.id}_competence_matrices_v1`, {}),
            phases: readJsonFromStorage(`forge_project_${project.id}_phases_v1`, []),
    ...
    reader.onload = () => {
        try {
            ...
            payload.projects.forEach((project) => {
                ...
                localStorage.setItem(`forge_project_${project.id}_context_v1`, JSON.stringify(data.context || structuredClone(CONTEXT_DEFAULT)));
                localStorage.setItem(`forge_project_${project.id}_stakeholders_v1`, JSON.stringify(data.stakeholders || []));
                localStorage.setItem(`forge_project_${project.id}_competence_categories_v1`, JSON.stringify(data.competenceCategories || structuredClone(defaultCompetenceCategories)));
                localStorage.setItem(`forge_project_${project.id}_competence_matrices_v1`, JSON.stringify(data.competenceMatrices || {}));
                localStorage.setItem(`forge_project_${project.id}_phases_v1`, JSON.stringify(data.phases || []));
...
function exportActiveProjectToExcel() {
    ...
    const contextRowsForExcel = [["Section", "Contenu"]];
    Object.entries(contextFieldLabels).forEach(([key, label]) => {
        contextRowsForExcel.push([label, exportContext[key] || ""]);
    });
    ...
    const workbookBlob = buildForgeXlsxWorkbook([
        { name: "Infos projet", rows: projectInfoRows, widths: [22, 42], freezeHeader: true, autoFilter: true },
        { name: "Contexte", rows: contextRowsForExcel, widths: [28, 100], freezeHeader: true, autoFilter: true },
        { name: "Compétences", rows: competenceRowsForExcel, widths: [24, 34, ...exportStakeholders.map(() => 18)], freezeHeader: true, autoFilter: true },
        { name: "SWOT", rows: swotRows, widths: [22, 72], freezeHeader: true, autoFilter: true },
        { name: "Types risques", rows: riskTypeRows, widths: [8, 28], freezeHeader: true, autoFilter: true },
        { name: "Risques", rows: riskRowsForExcel, widths: [8, 22, 42, 42, 14, 14, 14, 18, 24, 64], freezeHeader: true, autoFilter: true },
...
function forgeBootstrap() {
    ...
    if (currentPage === "context") initContextPage();
    if (V44_REDACTION_PAGES.includes(currentPage)) initRedactionPage(currentPage);
    if (currentPage === "competences") initCompetencesPage();
    if (currentPage === "stakeholders") initStakeholdersPage();
    if (currentPage === "swot") initSwotPage();
    if (currentPage === "risks") initRisksPage();
...
function renderDashboardHero(data, stats) {
    ...
    if (dates) {
        const start = data.project?.startDate || "début non renseigné";
        const end = data.project?.endDate || "fin non renseignée";
        dates.textContent = `Période : ${start} → ${end}`;
    }
...
function renderDashboardKpis(stats) {
    ...
    const cards = [
        ["Santé projet", `${stats.healthScore}%`, `${stats.errors} bloquant(s), ${stats.warnings} alerte(s)`],
        ["Contrôles", stats.checks, "points détectés"],
        ["Objectifs", stats.objectives, `${stats.smartRows} ligne(s) SMART`],
        ["Avancement WBS", `${stats.averageWbsProgress}%`, `${stats.wbsTasks} tâche(s)`],
        ["Risques critiques", stats.criticalRisks, `${stats.risks} risque(s) au total`],
        ["KPIs incomplets", stats.kpisIncomplete, `${stats.kpis} KPI(s) au total`]
    ];
    ...
    if (list) {
        list.innerHTML = checks.map((check) => `
            <article class="coherence-item coherence-severity-${escapeHtml(check.severity)}">
                <p class="coherence-item-title">${escapeHtml(check.title)}</p>
...
async function captureCardToClipboard(card, triggerButton) {
    ...
    try {
        ...
        const blob = await renderElementToPngBlob(card);

        try {
            if (!navigator.clipboard || typeof ClipboardItem === "undefined") {
                throw new Error("Clipboard image non disponible");
            }
            ...
            console.warn("Copie presse-papiers impossible, téléchargement PNG :", clipboardError);
            downloadCaptureBlob(blob, getCaptureFileName(card));
            showCaptureToast("Copie bloquée par le navigateur : PNG téléchargé ✅");
        }
    } catch (error) {
...
async function captureCardToClipboard(card, triggerButton) {
    ...
    try {
        ...
        try {
            ...
            console.warn("Copie presse-papiers impossible, téléchargement PNG :", clipboardError);
            downloadCaptureBlob(blob, getCaptureFileName(card));
            showCaptureToast("Copie bloquée : PNG téléchargé ✅");
        }
    } catch (error) {
        console.error("Capture impossible :", error);
        showCaptureToast("Capture impossible sur ce bloc.");
...
async function renderCardToExactVisualBlob(card) {
    ...
    try {
        ...
        const canvas = await window.html2canvas(card, {
            backgroundColor: getExactCaptureBackground(),
            scale,
            useCORS: true,
            allowTaint: true,
            logging: false,
...
function getExactCaptureScale(rect) {
    ...
    if (area > 2500000) return 1;
    if (area > 1200000) return 1.35;
...
function getExactCaptureBackground() {
    ...
    if (theme === "light") return "#eef7ff";
    if (theme === "orange") return "#1b130b";
...
async function renderCardToExactVisualBlob(card) {
    ...
    try {
        showCaptureToast("Capture HD en préparation…");
        await waitForCaptureLayout();

        const rect = card.getBoundingClientRect();
        const scale = getExactCaptureScale(rect);
        const background = getExactCaptureBackground();
...
function cleanForgeLocalStorage() {
    ...
    const firstConfirmation = confirm(
        `Cette action va supprimer ${keys.length} entrée(s) Forge du stockage local de ce navigateur.\n\n` +
    ...
    const secondConfirmation = confirm(
        "Dernière confirmation : nettoyer Forge va remettre l’application à zéro dans ce navigateur.\n\n" +
        "Tu confirmes ?"
    );
    ...
    alert("Stockage local Forge nettoyé. L’application va se recharger.");
...
async function initForgeDbBridge() {
    ...
    try {
        ...
        hydrateForgeStorageFromDb(entries);
        patchForgeLocalStorageSync();

        forgeDbBridgeActive = true;
        console.info("Forge DB : stockage SQLite actif.");
    } catch (error) {
        forgeDbBridgeActive = false;
...
function hydrateForgeStorageFromDb(entries) {
...
function renderGantt(ganttHead, ganttBody, ganttSummary) {
    ...
    const days = buildDateRange(timelineStart, timelineEnd);
    const groupedTasks = groupGanttTasksByPhase(visibleTasks);
    const totalColumns = 5 + days.length;
...
function renderGantt(ganttHead, ganttBody, ganttSummary) {
    ...
    const groupedTasks = groupGanttTasksByPhase(visibleTasks);
    const visibleValidTasks = visibleTasks.filter((task) => task.start && task.end);
    const board = buildGanttGridBoard(groupedTasks, days);
...
function buildGanttGridBoard(groupedTasks, days) {
    ...
    return `
        <div class="gantt-v76-board" style="--gantt-days: ${days.length};">
            ...
            <div class="gantt-v76-timeline-scroll">
                <div class="gantt-v76-timeline" style="--gantt-days: ${days.length};">
                    ...
                    <div class="gantt-v76-weekday-row">
                        ${days.map((day) => `
                            <div class="gantt-v76-weekday-cell" title="${escapeHtml(formatForgeFullDateLabel(day))}">
                    ...
                    <div class="gantt-v76-date-row">
                        ${days.map((day) => `
                            <div class="gantt-v76-date-cell ${isSameForgeDay(day, startOfForgeDay(new Date())) ? "today" : ""}" title="${escapeHtml(formatForgeFullDateLabel(day))}">
                                ${String(day.getDate()).padStart(2, "0")}
                            </div>
                        `).join("")}
                    </div>
                    ${rows.map((row) => row.type === "phase" ? buildGanttGridTimelinePhase(row, days) : buildGanttGridTimelineTask(row, days)).join("")}
                </div>
            </div>
        </div>
    `;
...
function buildGanttGridTimelineTask(row, days) {
    const task = row.task;
    const progress = clampPercent(task.row.avancement);
    const activeDayIndexes = days
        .map((day, index) => isDateInGanttTask(day, task) ? index : null)
    ...
    return `
        <div class="gantt-v76-day-row" style="--gantt-color: ${escapeHtml(row.color)}; --gantt-color-soft: ${escapeHtml(hexToRgba(row.color, 0.22))}; --gantt-color-strong: ${escapeHtml(hexToRgba(row.color, 0.72))};">
            ${days.map((day, index) => {
                ...
                return `<div class="${classes}" title="${escapeHtml(task.row.task || "")} · ${formatForgeDate(day)}"></div>`;
            }).join("")}
        </div>
    `;
}
